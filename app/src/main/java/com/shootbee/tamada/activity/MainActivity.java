...
import com.shootbee.tamada.databinding.ActivityMainBinding;
...
import com.shootbee.tamada.util.ClipboardListener;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initViews();
        //loading banner with glide
        RequestOptions options_ = new RequestOptions()
                .placeholder(R.mipmap.ic_launcher_round)
                .error(R.mipmap.ic_launcher_round);
        ;
       // Glide.with(this).load(Utils.IMAGE_URL+options.get(position).getImageUrl()).apply(options_).into(holder.imgEmoji);
        binding.rvEmoji.setLayoutManager(new LinearLayoutManager(this, LinearLayout.HORIZONTAL,false));
        refreshOptionsList();
    }
    ...
    private void refreshOptionsList()
    {
        ...
        commonClassForAPI.getOptionsList(Utils.BASE_URL+"options", new Callback<List<Option>>() {
            @Override
            public void onResponse(Call<List<Option>> call, Response<List<Option>> response) {
                Utils.hideProgressDialog(MainActivity.this);
                loadBanner();
                if(response.isSuccessful() && response.code() == 200)
                {

                    emojiAdapter = new EmojisAdapter(MainActivity.this,response.body());
                    binding.rvEmoji.setAdapter(emojiAdapter);
                    emojiAdapter.notifyDataSetChanged();

                }
                else
                    Utils.setToast(MainActivity.this,"Error loading emoji List");
            }

            @Override
            public void onFailure(Call<List<Option>> call, Throwable t) {
                Utils.hideProgressDialog(MainActivity.this);
                Utils.setToast(MainActivity.this,t.getMessage());
                loadBanner();
            }
        });
    }
    ...
    {
        CommonClassForAPI commonClassForAPI = CommonClassForAPI.getInstance(this);
        commonClassForAPI.getBanner(Utils.BASE_URL + "banner", new Callback<List<Banner>>() {
            @Override
            public void onResponse(Call<List<Banner>> call, Response<List<Banner>> response) {
                if(response.isSuccessful() && response.code() == 200)
                {
                    RequestOptions options_ = new RequestOptions()
                            .placeholder(R.mipmap.ic_launcher_round)
                            .error(R.mipmap.ic_launcher_round);
                    Glide.with(MainActivity.this).load(Utils.IMAGE_URL+response.body().get(0).getBannerImage()).apply(options_).into(binding.imgBanner);
